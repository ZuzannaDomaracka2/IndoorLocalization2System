...
final public class MyApplication extends Application {
    ...
    public void setDeadReckoningAvailable(boolean available)
    {
        if(algorithmOptionList != null && !algorithmOptionList.isEmpty())
        {
            for (AlgorithmOption algorithmOption : algorithmOptionList)
            {
                if(algorithmOption.getName().equalsIgnoreCase(getString(R.string.dead_reckoning)))
                {
                    algorithmOption.setAvailable(available);
                    if (!available) algorithmOption.setEnabled(false);
                }
            }
        }
    }
